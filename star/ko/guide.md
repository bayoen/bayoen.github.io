...
<p align="center">
    ...
    <br/><img src="{{ site.lang_url }}/res/bayoen-star-instuction-menu-selected-ko.png" class="box" alt="bayoen-star-instuction-menu-selected"/>
        ...
        <br/><span><strong>오버레이</strong>기능으로 점수판을 게임위에 띄워서 쓸 수도 있어요!</span>
</p>
...
<p align="center">
    ...
    <br/><span>점수판 모드를 바꿔서 내가 원하는 점수를 표시할 수 있습니다</span>
...
이제 본격적으로 알아볼까요? 대부분의 **바요엔-스타** 기능들을 정리했습니다.
...
- 점수: **바요엔-스타**는 **뿌요뿌요 테트리스** 게임 플레이 중, 별이 오르는 순간의 점수변동을 계산하여 점수판에 갱신합니다.

- 저장: **바요엔-스타**에서는 점수와 설정을 실시간 저장합니다. **바요엔-스타**가 강제종료 되더라도 일부정보는 복구할 수 있습니다.
    - 점수 및 데이터: **바요엔-스타** 경로에 **data.json**으로 저장합니다.
    - 설정: **바요엔-스타** 경로에 **pref.json**으로 저장합니다.
...
- 메모리: (중요) **바요엔-스타**는 컴퓨터 메모리 후킹으로 동작하기 때문에, 두 가지 이슈가 있습니다
    - **뿌요뿌요 테트리스**의 동작이 느려질까?
        - **바요엔-스타**는 상당히 가볍게 만들어진 프로그램입니다. 따라서 **뿌요뿌요 테트리스** 실행의 권장사항을 만족한다면, 방송송출까지 성능저하없이 쓸 수 있을거에요!
        
        _지나치게 느려진다면 <a href="https://github.com/bayoen/bayoen-star-exe/issues" target="_blank"><strong>신고/건의</strong></a>에 신고해주세요!_
    - **뿌요뿌요 테트리스** 메모리를 읽는다면 보안은 괜찮을까?
        - **바요엔-스타**은 메모리 조작을 하지 않고, 메모리 읽기만 수행합니다.
        - **바요엔-스타**는 유저가 보는 것과 같이 게임에서 겉으로 드러나는 정보만 읽습니다.
        
        <!-- _단, 유저식별을 위해 **Steam ID**는 추가로 조회합니다_ -->
...
5. **점수판 모드**: 점수판의 유형을 바꿀 수 있습니다. 자세한 내용은 [**'Mode: 점수판 모드 설정'**](#Mode)을 봐주세요.
...
- **'Load'** 불러오기(갱신): 강제로 현재 **'스타+'**와 **'크라운'** 점수를 점수 모니터에 갱신합니다.
- **'Save'** 저장: **'스타+'**와 **'크라운'** 점수를 현재 값으로 저장합니다.

_인터넷 방송을 하시나요? 그럼 [**'방송설정'**]({{ site.lang_url }}/ko/streaming.html)도 확인해보세요!_
...
단축키는 매우 간단하게 구성했습니다. 추후 더 넣을 예정입니다.
...
- 오버레이를 찾을 수 없나요?
- 점수판이 깨져서 초기화가 되지 않은가요?
- 알 수 없는 오작동이 있나요?

**바요엔-스타**를 깔끔하게 초기화해서 해결하세요!

1. 먼저 **바요엔-스타**를 종료합니다
1. **data.json, pref.json**이 있다면 지워주세요
1. 다시 **바요엔-스타**를 실행합니다
1. 끝!

_이후 업데이트에서 전체 초기화 기능을 포함시킬 계획입니다_
